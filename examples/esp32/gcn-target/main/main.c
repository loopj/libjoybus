...
// Mapping of button GPIOs to their corresponding bits in the GameCube controller input state
const unsigned int button_map[][2] = {
  {BUTTON_A_GPIO, JOYBUS_GCN_BUTTON_A},
};
  ...
  while (1) {
    ...
    for (int i = 0; i < sizeof(button_map) / sizeof(button_map[0]); i++) {
      if (gpio_get_level(button_map[i][0]) == 0) {
        gcn_controller.input.buttons |= button_map[i][1];
      }
    }
