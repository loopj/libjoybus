...
// Call the backend handler carried in the interrupt argument
static void joybus_rp2xxx_isr(const void *arg)
{
  ((irq_handler_t)(uintptr_t)arg)();
}

// Connect a backend handler dynamically, since the kernel owns the vector table
static void joybus_rp2xxx_set_irq_handler(uint irq_num, irq_handler_t handler)
{
  irq_connect_dynamic(irq_num, CONFIG_JOYBUS_RP2XXX_IRQ_PRIORITY, joybus_rp2xxx_isr, (const void *)(uintptr_t)handler,
                      0);
  irq_enable(irq_num);
}
